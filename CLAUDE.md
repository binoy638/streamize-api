...
| `torrents` | `Store` wraps all SQLite reads/writes for torrents, torrent files, subtitles, and preview assets. Torrent status constants live here (`added`, `downloading`, `done`, `error`, etc.). |
...
React + Vite app. Dev server proxies `/api` to `:8080`. The sign-in screen has a prototype bypass for browsing mock media flows when the API isn't running. Pages: `TorrentsPage`, `TorrentDetailPage`, `LibraryPage`, `PlayerPage`.
